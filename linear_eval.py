...
from src.utils import AllReduce, init_distributed, WarmupCosineSchedule
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        def train_step():
            ...
            for i, data in enumerate(data_loader):
                ...
                top5_correct += float(
                    outputs.topk(5, dim=1).indices.eq(labels.unsqueeze(1)).sum()
                )
            ...
            return 100.0 * top1_correct / total
...
def load_pretrained(r_path, encoder, linear_classifier, device_str):
    ...
    if linear_classifier is not None:
        pretrained_dict = {
            k.replace("module.", ""): v for k, v in checkpoint["classifier"].items()
        }
...
def load_from_path(r_path, encoder, linear_classifier, opt, sched, device_str):
    encoder, linear_classifier = load_pretrained(
        r_path, encoder, linear_classifier, device_str
    )
...
def init_model(
    ...
    param_groups = [
        {
            "params": (
                p
                for n, p in linear_classifier.named_parameters()
    ...
    optimizer = SGD(
        param_groups, nesterov=True, weight_decay=weight_decay, momentum=0.9, lr=ref_lr
    )
